...
class Speaker:
    ...
    def get_unknown_info(self, diarization_df: pd.DataFrame, known_ids, unknown_id):
        ...
        self.audio_speakers = diarization_df['speaker'].unique().tolist()
        ...
        others_count = len(self.audio_speakers) - len(known_ids)
        self.other_IDs = []
    ...
    def retreated_dataframe(self, diarization_df, episod_id, unknown_id, start_ep):
        ...
        if not self.other_speakers:
            diarization_df['speaker'].replace(self.speakers_mapping[unknown_id],unknown_id,inplace = True) # replace the target name, if only 1 unknown (ie. other_speakers list is empty)
        for i in range(len(self.speaker_ids)-1):
            diarization_df['speaker'].replace(self.speakers_mapping[self.speaker_ids[i]],self.speaker_ids[i],inplace = True) # replace the known speakers
        for i in range(len(self.other_speakers)):
            diarization_df['speaker'].replace(self.otherIDs_mapping[self.other_IDs[i]],self.other_IDs[i],inplace = True) # replace the other speakers
